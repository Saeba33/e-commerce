...
- Gérer la quantité au niveau de la carte et du panier (avec impact sur le stock) :
 Permettre le click sur le bouton "+" tant que la quantité sélectionnée par l'utilisateur est inférieure à mon stock. Il faut un       verrouillage front mais aussi back (dans le controller). Il faut également générer une ligne dans StockHistory. Il faut donc ajouter un champ "origin" dans stock history pour savoir si la modification a été apportée par un utilisateur ou par une commande, le cas échéant, reprendre le numéro de commande.

<!-- - Ajouter la possibilité de modifier le stock dans la modification de produit. Cela sera notamment utile si l'utilisateur s'est trompé au niveau de l'ajout du stock. Cibler la modification dans StockHistory au niveau du noouveau champ "origin". 
Si on doit résumer l'impact sur Sotck History, on a un nouveau champ "origin" qui reprend l'incrémentation ou la décrémentation du stock en fonction de : commande validée, modification via le bouton "ajouter du stock", modification directe du stock depuis l'édition de la carte produit. -->
